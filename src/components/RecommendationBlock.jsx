...
export default function RecommendationBlock({ stepKey, brandName, body, lang }) {
  ...
  return (
    <div style={{ backgroundColor: '#FFF8E1', borderLeft: '3px solid #FED44B', borderRadius: 10, padding: '16px 20px' }}
      className="mb-3">
      <div className="flex items-start gap-3">
        <Sparkles size={16} style={{ color: '#FED44B' }} className="mt-0.5 shrink-0" />
        <div className="flex-1">
          <div className="text-[11px] font-bold uppercase tracking-wider mb-1" style={{ color: '#92400E' }}>
            {t ? `Recommandation pour ${brandName || 'votre marque'}` : `Recommendation for ${brandName || 'your brand'}`}
          </div>
          <p className="text-[13px] text-[#645648] leading-relaxed">{body}</p>
        </div>
