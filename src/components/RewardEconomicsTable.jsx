...
export default function RewardEconomicsTable({ rewards, setRewards, tiers, tierStats, burnRate, lang }) {
  ...
  return (
    <div className="bg-[#EEEDE6] rounded-xl border border-[#D9D5CB] p-5">
      <h3 className="font-semibold text-[#52473C] text-sm mb-4">{t.title}</h3>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[#D9D5CB]">
              <th className="text-left py-2 px-2 text-[#8A7D6B] font-medium">{t.name}</th>
              <th className="text-center py-2 px-2 text-[#8A7D6B] font-medium">{t.usage}</th>
              <th className="text-center py-2 px-2 text-[#8A7D6B] font-medium">{t.pointsCost}</th>
              <th className="text-center py-2 px-2 text-[#8A7D6B] font-medium">{t.realCost}</th>
              {tiers.map((tier, i) => (
                <th key={i} className="text-center py-2 px-1 font-medium" style={{ color: tier.color }}>
                  {tier.name}
                </th>
              ))}
              <th className="text-right py-2 px-2 text-[#8A7D6B] font-medium">{t.annualCost}</th>
              <th className="w-8" />
            </tr>
          </thead>
          <tbody>
            {rewards.map(r => (
              <tr key={r.id} className="border-b border-[#E5E1D8] hover:bg-[#EEEDE6]/50">
                <td className="py-2 px-2">
                  <input
                    type="text"
                    value={lang === 'fr' ? r.nameFr : r.nameEn}
                    onChange={e => updateReward(r.id, lang === 'fr' ? 'nameFr' : 'nameEn', e.target.value)}
                    className="w-full px-1.5 py-1 border border-transparent hover:border-[#D9D5CB] focus:border-primary rounded text-xs bg-transparent min-w-[120px]"
                  />
                </td>
                <td className="py-2 px-2 text-center">
                  <select
                    ...
                    className="px-1.5 py-1 border border-[#D9D5CB] rounded text-[10px] bg-[#EEEDE6]"
                  >
                ...
                <td className="py-2 px-2 text-center">
                  {(r.rewardUsage === 'burn' || r.rewardUsage === 'both') ? (
                    <input
                      ...
                      className="w-16 px-1.5 py-1 border border-[#D9D5CB] rounded text-[10px] text-center"
                    />
                  ) : (
                ...
                <td className="py-2 px-2 text-center">
                  <div className="inline-flex items-center">
                    <input
                      ...
                      className="w-14 px-1.5 py-1 border border-[#D9D5CB] rounded text-[10px] text-center"
                    />
                    <span className="text-[10px] text-[#8A7D6B] ml-0.5">€</span>
                  </div>
                </td>
                {tiers.map((tier, i) => (
                ...
                <td className="py-2 px-2 text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <span className="font-medium text-[#645648]">{formatCurrency(estimateAnnualCost(r))}</span>
                  </div>
                </td>
                <td className="py-2 px-1">
                  <button onClick={() => removeReward(r.id)} className="text-[#8A7D6B] hover:text-danger transition-colors">
                    <X size={12} />
                  </button>
                </td>
              </tr>
          ...
          <tfoot>
            <tr className="border-t-2 border-[#D9D5CB]">
              <td colSpan={4 + tiers.length} className="py-2 px-2 text-right font-semibold text-[#645648]">
                {t.total}
              </td>
      ...
      {/* Add reward row */}
      <div className="flex gap-2 items-end mt-4 pt-3 border-t border-[#D9D5CB]">
        <input
          type="text" value={newReward.name}
          onChange={e => setNewReward(p => ({ ...p, name: e.target.value }))}
          placeholder={t.placeholder}
          className="flex-1 px-2 py-1.5 border border-[#D9D5CB] rounded-lg text-xs"
        />
        <select
          value={newReward.rewardUsage}
          onChange={e => setNewReward(p => ({ ...p, rewardUsage: e.target.value }))}
          className="px-2 py-1.5 border border-[#D9D5CB] rounded-lg text-xs"
        >
          {REWARD_USAGE_OPTIONS.map(opt => (
            <option key={opt.value} value={opt.value}>
              {opt.emoji} {lang === 'fr' ? opt.labelFr : opt.labelEn}
        ...
        <input
          ...
          className="w-14 px-2 py-1.5 border border-[#D9D5CB] rounded-lg text-xs text-center"
          placeholder="€"
        />
        <button
